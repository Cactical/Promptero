...
To use this with a c++ program, put #include <iostream> + <signal.h> + "Promptero.hpp", and any other header files that a command, func, process, etc in here uses, then call the activate() function in XXXX to start Promptero.
